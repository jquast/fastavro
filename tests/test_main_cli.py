...
def test_cli_record_output():
    ...
    given_cmd_args = ['fastavro', given_avro_input]
    expected_data = [
...
def test_cli_arg_metadata():
    ...
    given_cmd_args = ['fastavro', '--metadata', given_avro_input]
...
def test_cli_arg_schema():
    # given,
    given_avro_input = os.path.join(data_dir, 'weather.avro')
    given_cmd_args = ['fastavro', '--schema', given_avro_input]
    expected_schema = {
        'type': 'record',
        'name': 'Weather',
        'namespace': 'test',
...
def test_cli_arg_codecs():
    # given,
    given_cmd_args = ['fastavro', '--codecs']
    ...
    # verify
    data = sorted(filter(None, map(str.strip, result_output.splitlines())))
